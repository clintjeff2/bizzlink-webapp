...
export default function ProfilePage() {
  ...
  const [updateUserCertifications] = useUpdateUserCertificationsMutation()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.certifications && JSON.stringify(formData.certifications) !== JSON.stringify(profileData?.certifications)) {
        await updateUserCertifications({
          userId: user.userId,
  ...
  const addEmployment = () => {
    const newEmployment: UserEmployment = {
      id: Date.now(),
      companyName: '',
      jobTitle: '',
      city: '',
  ...
  const removeEmployment = (employmentId: number) => {
    setFormData(prev => ({
      ...prev,
      employment: prev.employment?.filter(emp => emp.id !== employmentId) || []
    }))
  }
  ...
  const addProject = () => {
    const newProject: UserPortfolio = {
      id: Date.now().toString(),
      title: '',
      description: '',
      imageLink: '',
  ...
  const removeProject = (projectId: string) => {
    setFormData(prev => ({
      ...prev,
      portfolio: prev.portfolio?.filter(project => project.id !== projectId) || []
    }))
  }
                ...
                <Card className="shadow-lg">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      {isEditing && (
                        <Button size="sm" onClick={addEmployment}>
                          <Plus className="w-4 h-4 mr-2" />
                          Add Experience
                        </Button>
                      )}
                        ...
                        <div key={emp.id || index} className="border-l-2 border-primary-blue/30 pl-4 relative">
                          ...
                          {isEditing ? (
                            <div className="space-y-4 border rounded-lg p-4 bg-gray-50">
                              <div className="flex items-center justify-between">
                                <h4 className="font-medium text-gray-900">Edit Experience</h4>
                                <Button 
                                  size="sm" 
                                  variant="outline" 
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  <Label htmlFor={`emp-title-${emp.id}`}>Job Title</Label>
                                  <Input
                                    id={`emp-title-${emp.id}`}
                                    value={emp.jobTitle || ''}
                                    onChange={(e) => {
                                      const updatedEmployment = [...(formData.employment || [])]
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], jobTitle: e.target.value }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                                ...
                                <div>
                                  <Label htmlFor={`emp-company-${emp.id}`}>Company</Label>
                                  <Input
                                    id={`emp-company-${emp.id}`}
                                    value={emp.companyName || ''}
                                    onChange={(e) => {
                                      const updatedEmployment = [...(formData.employment || [])]
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], companyName: e.target.value }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                                ...
                                <div>
                                  <Label htmlFor={`emp-city-${emp.id}`}>City</Label>
                                  <Input
                                    id={`emp-city-${emp.id}`}
                                    value={emp.city || ''}
                                    onChange={(e) => {
                                      ...
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], city: e.target.value }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                                    placeholder="City"
                                ...
                                <div>
                                  <Label htmlFor={`emp-country-${emp.id}`}>Country</Label>
                                  <Input
                                    id={`emp-country-${emp.id}`}
                                    value={emp.country || ''}
                                    onChange={(e) => {
                                      const updatedEmployment = [...(formData.employment || [])]
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], country: e.target.value }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                                    placeholder="Country"
                                ...
                                <div>
                                  <Label htmlFor={`emp-start-${emp.id}`}>Start Date</Label>
                                  <Input
                                    id={`emp-start-${emp.id}`}
                                    type="month"
                                    value={emp.startDate || ''}
                                    onChange={(e) => {
                                      ...
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], startDate: e.target.value }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                                  />
                                ...
                                <div>
                                  <Label htmlFor={`emp-end-${emp.id}`}>End Date</Label>
                                  <Input
                                    id={`emp-end-${emp.id}`}
                                    type="month"
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                      if (empIndex !== -1) {
                                        updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], endDate: e.target.value, current: false }
                                        setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                      }
                                    }}
                              ...
                              <div className="flex items-center space-x-2">
                                <input
                                  type="checkbox"
                                  id={`emp-current-${emp.id}`}
                                  checked={emp.current || false}
                                  onChange={(e) => {
                                    const updatedEmployment = [...(formData.employment || [])]
                                    const empIndex = updatedEmployment.findIndex(emp => emp.id === emp.id)
                                    if (empIndex !== -1) {
                                      updatedEmployment[empIndex] = { 
                                        ...updatedEmployment[empIndex], 
                                        current: e.target.checked,
                                        endDate: e.target.checked ? '' : updatedEmployment[empIndex].endDate
                                      }
                                      setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                    }
                                  }}
                                  className="rounded"
                                />
                                <Label htmlFor={`emp-current-${emp.id}`}>I currently work here</Label>
                              </div>
                              <div>
                                <Label htmlFor={`emp-desc-${emp.id}`}>Description</Label>
                                <Textarea
                                  id={`emp-desc-${emp.id}`}
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const empIndex = updatedEmployment.findIndex(e => e.id === emp.id)
                                    if (empIndex !== -1) {
                                      updatedEmployment[empIndex] = { ...updatedEmployment[empIndex], description: e.target.value }
                                      setFormData(prev => ({ ...prev, employment: updatedEmployment }))
                                    }
                                  }}
                            ...
                            <div className="flex items-start justify-between mb-2">
                              <div>
                                <h3 className="font-semibold text-gray-900">{emp.jobTitle}</h3>
                                <p className="text-primary-blue font-medium">{emp.companyName}</p>
                                <p className="text-sm text-gray-600">{emp.city}, {emp.country}</p>
                                <p className="text-gray-700 text-sm mt-2">{emp.description}</p>
                              </div>
                      ...
                      {(displayData.education || []).map((edu: any, index: number) => (
                        <div key={edu.id || index} className="border-l-2 border-primary-green/30 pl-4">
                          {isEditing ? (
                            <div className="space-y-4 border rounded-lg p-4 bg-gray-50">
                              <div className="flex items-center justify-between">
                                <h4 className="font-medium text-gray-900">Edit Education</h4>
                                <Button 
                                  size="sm" 
                                  variant="outline" 
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], schoolName: e.target.value }
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], location: e.target.value }
                                        setFormData(prev => ({ ...prev, education: updatedEducation }))
                                      }
                                    }}
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], degree: e.target.value }
                                        setFormData(prev => ({ ...prev, education: updatedEducation }))
                                      }
                                    }}
                                ...
                                <div>
                                  ...
                                  <Input
                                    id={`edu-field-${edu.id}`}
                                    value={edu.studyField || ''}
                                    onChange={(e) => {
                                      const updatedEducation = [...(formData.education || [])]
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], studyField: e.target.value }
                                        setFormData(prev => ({ ...prev, education: updatedEducation }))
                                      }
                                    }}
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], startDate: e.target.value }
                                        setFormData(prev => ({ ...prev, education: updatedEducation }))
                                      }
                                    }}
                                  />
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                      if (eduIndex !== -1) {
                                        updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], endDate: e.target.value }
                                        setFormData(prev => ({ ...prev, education: updatedEducation }))
                                      }
                                    }}
                                  />
                              ...
                              <div>
                                ...
                                <Textarea
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const eduIndex = updatedEducation.findIndex(e => e.id === edu.id)
                                    if (eduIndex !== -1) {
                                      updatedEducation[eduIndex] = { ...updatedEducation[eduIndex], description: e.target.value }
                                      setFormData(prev => ({ ...prev, education: updatedEducation }))
                                    }
                                  }}
                            ...
                            <div className="flex items-start justify-between mb-1">
                              <div>
                                ...
                                <p className="text-primary-green font-medium">{edu.schoolName}</p>
                                <p className="text-sm text-gray-600">{edu.location}</p>
                                <p className="text-sm text-gray-600">{edu.studyField}</p>
                        ...
                        <div key={cert.id || index} className="border border-gray-200 rounded-lg p-4">
                          {isEditing ? (
                            <div className="space-y-4 bg-gray-50 p-4 rounded-lg">
                              <div className="flex items-center justify-between">
                                <h4 className="font-medium text-gray-900">Edit Certification</h4>
                                <Button 
                                  size="sm" 
                                  variant="outline" 
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], name: e.target.value }
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], issuer: e.target.value }
                                        setFormData(prev => ({ ...prev, certifications: updatedCertifications }))
                                      }
                                    }}
                                ...
                                <div>
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], credentialId: e.target.value }
                                        setFormData(prev => ({ ...prev, certifications: updatedCertifications }))
                                      }
                                    }}
                                ...
                                <div>
                                  <Label htmlFor={`cert-issue-${cert.id}`}>Issue Date</Label>
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], issueDate: e.target.value }
                                        setFormData(prev => ({ ...prev, certifications: updatedCertifications }))
                                      }
                                    }}
                                  />
                                ...
                                <div>
                                  <Label htmlFor={`cert-expire-${cert.id}`}>Expiration Date (Optional)</Label>
                                  <Input
                                    id={`cert-expire-${cert.id}`}
                                    type="month"
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], expirationDate: e.target.value }
                                        setFormData(prev => ({ ...prev, certifications: updatedCertifications }))
                                      }
                                    }}
                                ...
                                <div className="md:col-span-2">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const certIndex = updatedCertifications.findIndex(c => c.id === cert.id)
                                      if (certIndex !== -1) {
                                        updatedCertifications[certIndex] = { ...updatedCertifications[certIndex], credentialUrl: e.target.value }
                                        setFormData(prev => ({ ...prev, certifications: updatedCertifications }))
                                      }
                                    }}
                                    placeholder="https://..."
                      ...
                      <Card key={project.id || index} className="shadow-lg overflow-hidden">
                        {isEditing ? (
                          <div className="p-4 space-y-4 bg-gray-50">
                            <div className="flex items-center justify-between">
                              <h4 className="font-medium text-gray-900">Edit Project</h4>
                              <Button 
                                size="sm" 
                                variant="outline" 
                            ...
                            <div className="space-y-4">
                              <div>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    if (projIndex !== -1) {
                                      updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], title: e.target.value }
                              ...
                              <div>
                                ...
                                <Textarea
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const projIndex = updatedPortfolio.findIndex(p => p.id === project.id)
                                    if (projIndex !== -1) {
                                      updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], description: e.target.value }
                                      setFormData(prev => ({ ...prev, portfolio: updatedPortfolio }))
                                    }
                                  }}
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  <Label htmlFor={`proj-role-${project.id}`}>Your Role</Label>
                                  <Input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const projIndex = updatedPortfolio.findIndex(p => p.id === project.id)
                                      if (projIndex !== -1) {
                                        updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], role: e.target.value }
                                        setFormData(prev => ({ ...prev, portfolio: updatedPortfolio }))
                                      }
                                    }}
                                ...
                                <div>
                                  <Label htmlFor={`proj-date-${project.id}`}>Completion Date</Label>
                                  <Input
                                    id={`proj-date-${project.id}`}
                                    type="month"
                                    ...
                                    onChange={(e) => {
                                      ...
                                      const projIndex = updatedPortfolio.findIndex(p => p.id === project.id)
                                      if (projIndex !== -1) {
                                        updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], completionDate: e.target.value }
                                        setFormData(prev => ({ ...prev, portfolio: updatedPortfolio }))
                                      }
                                    }}
                                  />
                              ...
                              <div>
                                ...
                                <Input
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const projIndex = updatedPortfolio.findIndex(p => p.id === project.id)
                                    if (projIndex !== -1) {
                                      updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], projectUrl: e.target.value }
                                      setFormData(prev => ({ ...prev, portfolio: updatedPortfolio }))
                                    }
                                  }}
                                  placeholder="https://..."
                              ...
                              <div>
                                <Label htmlFor={`proj-image-${project.id}`}>Image URL (Optional)</Label>
                                <Input
                                  id={`proj-image-${project.id}`}
                                  type="url"
                                  ...
                                  onChange={(e) => {
                                    ...
                                    const projIndex = updatedPortfolio.findIndex(p => p.id === project.id)
                                    if (projIndex !== -1) {
                                      updatedPortfolio[projIndex] = { ...updatedPortfolio[projIndex], imageLink: e.target.value }
                                      setFormData(prev => ({ ...prev, portfolio: updatedPortfolio }))
                                    }
                                  }}
                                  placeholder="https://..."

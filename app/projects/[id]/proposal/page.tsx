...
import { AlertCircle, ArrowLeft, Calendar, Clock, DollarSign, Eye, FileText, MessageSquare, Paperclip, Send, X } from "lucide-react"
import { Navigation } from "@/components/navigation"
import { useToast } from "@/hooks/use-toast"
import { useSelector } from "react-redux"
import { RootState } from "@/lib/redux/store"
import { useGetProjectQuery, useCreateProposalMutation, useGetProposalQuery, useUpdateProposalMutation } from "@/lib/redux/api/firebaseApi"
import { Proposal, ProposalBid } from "@/lib/redux/types/firebaseTypes"
import Link from "next/link"
import { Progress } from "@/components/ui/progress"
import { Badge } from "@/components/ui/badge"
import { validateProposalFile, uploadProposalAttachments } from "@/lib/services/storageService"
...
export default function ProposalPage() {
  ...
  const { data: project, isLoading: isLoadingProject } = useGetProjectQuery(id as string)
  
  // Fetch existing proposal data if in edit mode
  const { data: existingProposal, isLoading: isLoadingProposal } = useGetProposalQuery(
  ...
  const [existingAttachments, setExistingAttachments] = useState<{name: string, url: string, size: number, type: string}[]>([])
  ...
  const [createProposal, { isLoading: isCreating, isSuccess: isCreateSuccess, isError: isCreateError, error: createError }] = useCreateProposalMutation()
  const [updateProposal, { isLoading: isUpdating, isSuccess: isUpdateSuccess, isError: isUpdateError, error: updateError }] = useUpdateProposalMutation()
  ...
  useEffect(() => {
    if (isEditMode && existingProposal && !isLoadingProposal) {
      ...
      if (existingProposal.questions && existingProposal.questions.length > 0) {
        setQuestions(existingProposal.questions.map((q: any, index: number) => ({
          id: `existing-${index}`,
      ...
      if (existingProposal.attachments && existingProposal.attachments.length > 0) {
        setExistingAttachments(existingProposal.attachments.map(attachment => ({
  ...
  useEffect(() => {
    if (isError) {
      toast({
        title: "Error Submitting Proposal",
        description: (error as any)?.data?.message || "Something went wrong. Please try again.",
        variant: "destructive"
      })
    }
  }, [isError, error, toast])
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (fileList) {
      ...
      const totalExistingFiles = attachments.length + existingAttachments.length;
      if (totalExistingFiles + newFiles.length > 5) {
        setUploadError(`You can only upload a maximum of 5 files total. You've selected ${newFiles.length} new files, but can only add ${5 - totalExistingFiles} more.`)
        return
      }
  ...
  const handleAddQuestion = () => {
    if (newQuestion.trim() && questions.length < 5) {
      ...
      setQuestions(prev => [...prev, { id: questionId, question: newQuestion.trim() }])
      setNewQuestion("")
    }
  }
  ...
  // Track form changes
  useEffect(() => {
    if (bidAmount || timeline || coverLetter || attachments.length > 0 || questions.length > 0) {
      setFormTouched(true)
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/40">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8">The project you're applying to doesn't exist or has been removed.</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-8">
          ...
          <p className="text-lg text-gray-600">
            {isEditMode ? "Update" : "Submit"} your proposal for <span className="font-semibold text-blue-700">{project.title}</span>
          </p>
        </div>
            ...
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm overflow-hidden">
              ...
              <CardContent className="p-8 pt-6">
                ...
                <div className="space-y-6">
                  {/* Bid Details */}
                  <div>
                    <h3 className="text-base font-medium text-gray-900 mb-4">Bid Details</h3>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                      <div className="space-y-2">
                        <Label htmlFor="bid-type">Bid Type</Label>
                        <Select value={bidType} onValueChange={(value: "fixed" | "hourly") => setBidType(value)}>
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="bid-amount" className="flex justify-between">
                          <span>Bid Amount</span>
                          <span className="text-gray-500 text-sm">
                            {suggestedBid.currency}{suggestedBid.min} - {suggestedBid.currency}{suggestedBid.max} (suggested)
                          </span>
                        </Label>
                        <div className="relative">
                          ...
                          <Input
                            type="number"
                            id="bid-amount"
                            placeholder={bidType === "fixed" ? "Enter your bid amount" : "Enter hourly rate"}
                            className="pl-10"
                            value={bidAmount}
                        ...
                        {submitAttempted && !bidAmount && (
                          <p className="text-sm text-red-600">Please enter a bid amount</p>
                        )}
                      </div>
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="timeline" className="flex justify-between">
                        <span>Estimated Timeline</span>
                        <span className="text-gray-500 text-sm">
                          Project duration: {project.timeline?.duration || "Not specified"}
                        </span>
                      </Label>
                      <div className="relative">
                        <div className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                      ...
                      {submitAttempted && !timeline && (
                        <p className="text-sm text-red-600">Please specify your estimated timeline</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-base font-medium text-gray-900">Cover Letter</h3>
                    ...
                    <Textarea
                      ...
                      required
                    />
                    {submitAttempted && (!coverLetter || coverLetter.length < 50) && (
                      <p className="mt-2 text-sm text-red-600">
                        Please write a detailed cover letter (at least 50 characters)
                      </p>
                    )}
                    ...
                    <div className="mt-3 text-sm text-gray-500">
                      ...
                      <ul className="list-disc pl-5 mt-1 space-y-1">
                        <li>Show your understanding of the project requirements</li>
                        <li>Highlight your relevant skills and experience</li>
                        <li>Explain your approach to the project</li>
                        <li>Mention your availability and communication style</li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center justify-center w-full">
                        <label htmlFor="file-upload" className="flex flex-col items-center justify-center w-full h-32 border-2 border-blue-300 border-dashed rounded-lg cursor-pointer bg-blue-50 hover:bg-blue-100 transition-colors">
                          <div className="flex flex-col items-center justify-center pt-5 pb-6">
                            ...
                            <p className="text-xs text-blue-600">PDF, DOCX, JPG, PNG, GIF, or ZIP (Max 5MB)</p>
                          ...
                          <input 
                            ...
                            disabled={(attachments.length + existingAttachments.length) >= 5}
                          />
                        </label>
                      </div>
                      ...
                      <p className="text-sm text-gray-500">
                        Attach up to 5 files that showcase your relevant work or provide additional information.
                      </p>
                        ...
                        <div className="mt-4 space-y-2">
                          ...
                          <div className="space-y-3">
                            {existingAttachments.map((file, index) => (
                              <div key={`existing-${index}`} className="flex items-center justify-between p-3 bg-green-50 border border-green-100 rounded-lg">
                                <div className="flex items-center space-x-3 flex-1 mr-4">
                                  <div className="p-2 bg-white rounded-md shadow-sm">
                                    <FileText className="w-5 h-5 text-green-600" />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <p className="text-sm font-medium text-gray-800 truncate">{file.name || 'Unnamed file'}</p>
                        ...
                        <div className="mt-4 space-y-2">
                          ...
                          <div className="space-y-3">
                            {attachments.map((file, index) => (
                              <div key={`new-${index}`} className="flex items-center justify-between p-3 bg-blue-50 border border-blue-100 rounded-lg">
                                <div className="flex items-center space-x-3 flex-1 mr-4">
                                  <div className="p-2 bg-white rounded-md shadow-sm">
                                ...
                                {uploadProgress[file.name] !== undefined && uploadProgress[file.name] < 100 && (
                                  <div className="w-24 mr-4">
                                    <Progress value={uploadProgress[file.name]} className="h-2 bg-blue-100" />
                          ...
                          {(attachments.length + existingAttachments.length) >= 5 && (
                            <p className="text-sm text-amber-600">Maximum 5 files allowed</p>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-4">
                      <p className="text-sm text-gray-600">
                        Add up to 5 questions for the client to answer. This helps clarify project details and demonstrates your engagement.
                      </p>
                      ...
                      {questions.length >= 5 && (
                        <p className="text-sm text-amber-600">Maximum 5 questions allowed</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-blue-50 p-6 rounded-xl border border-blue-100 mt-8">
                    <div className="flex items-center space-x-3">
                      ...
                      <Label htmlFor="terms" className="text-sm text-gray-700">
                        I agree to the <Link href="/terms" className="text-blue-600 hover:underline">Terms and Conditions</Link> and confirm that my proposal is accurate and honest
                      </Label>
                    </div>
                  ...
                  <div className="pt-8 pb-2">
                    <Button 
                      className="w-full bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white font-bold py-5 text-lg shadow-lg hover:shadow-xl transition-all duration-200 rounded-xl"
                    ...
                    >
                      {isSubmitting ? (
                        <>
                          ...
                          {isEditMode ? "Updating Proposal..." : "Submitting Proposal..."}
                        </>
                      ) : (
                        <>
                    ...
                    {submitAttempted && validateForm() && (
                      <p className="mt-2 text-center text-red-600">{validateForm()}</p>
                    )}
                  </div>
                </div>
              </CardContent>
          ...
          <div className="space-y-6">
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm overflow-hidden">
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="pt-2">
                    <Link href={`/projects/${project.projectId}`} className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-gray-900">{project.timeline?.duration || "Not specified"}</span>
                    </div>
                    
                    <div className="flex justify-between">
                      <span className="text-gray-600 text-sm">Experience:</span>
                      <span className="font-semibold text-gray-900 capitalize">{project.requirements?.experienceLevel || "Any"}</span>
                    </div>
            ...
            <Card className="shadow-xl border-0 bg-white/90 backdrop-blur-sm overflow-hidden">
              <div className="bg-gradient-to-r from-emerald-600 to-teal-600 p-5">
                <h3 className="text-lg font-bold text-white">Submission Tips</h3>
              </div>
              <CardContent className="p-6">
                ...
                <div className="space-y-4 text-sm">
                  <div className="flex items-start space-x-3">
                    ...
                    <p className="text-gray-700">
                      <strong className="text-gray-900">Be specific.</strong> Show that you understand the project by mentioning specific requirements.
                    </p>
                  </div>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <p className="text-gray-700">
                      <strong className="text-gray-900">Be realistic.</strong> Set fair pricing and timeline expectations for the work required.
                    </p>
                  </div>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <p className="text-gray-700">
                      <strong className="text-gray-900">Stand out.</strong> Highlight your unique skills and experiences relevant to this project.
                    </p>
                  </div>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <p className="text-gray-700">
                      <strong className="text-gray-900">Proofread.</strong> Check for typos, grammar mistakes, and clarity before submitting.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            {formTouched && !agreeToTerms && !submitAttempted && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-start space-x-3">
                ...
                <p className="text-sm text-amber-800">
                  Don't forget to agree to the terms and conditions before submitting your proposal.
                </p>
              </div>
            )}
          </div>
